...
const chickenMediaInput = ref(null)
const activeConfirmPrompt = ref(null)
const uploadedChickenImages = ref([])
...
const selectedChickenMedia = computed(() => {
  if (uploadedChickenImages.value.length === 0) {
    return []
  }

  if (!hasChickenBreastUnlock.value) {
    return [uploadedChickenImages.value[0]]
  }

  return uploadedChickenImages.value
})
const effectiveRenderLimit = computed(() => {
  if (selectedChickenMedia.value.length === 0) {
    return 0
  }
...
const triggerChickenMediaUpload = () => {
  chickenMediaInput.value?.click()
}

const handleChickenMediaUploadSelection = async (event) => {
  ...
  if (imageFiles.length === 0) {
    window.alert('Please upload image files only.')
    return
  }

  try {
    const uploadedDataUrls = await Promise.all(imageFiles.map((file) => readFileAsDataUrl(file)))
    const uniqueImages = new Set([...uploadedChickenImages.value, ...uploadedDataUrls])
    uploadedChickenImages.value = Array.from(uniqueImages)
    syncRenderedChickens()
  } catch (error) {
    window.alert(`Could not upload media: ${error instanceof Error ? error.message : 'Unknown error'}`)
  }
}

const clearUploadedChickenMedia = () => {
  uploadedChickenImages.value = []
  syncRenderedChickens()
}
...
const createChicken = () => {
  ...
  const validImages = selectedChickenMedia.value
...
<template>
  <div class="black-screen" style="touch-action: none; user-select: none;">
    ...
    <div v-if="isMenuOpen" class="menu-panel">
      ...
      <div class="menu-bottom-section">
        ...
        <div class="menu-actions">
          <button type="button" class="menu-action-button" @click="triggerChickenMediaUpload">
            Upload chicken media ({{ uploadedChickenImages.length }})
          </button>
          <button
            type="button"
            class="menu-action-button"
            :disabled="uploadedChickenImages.length === 0"
            @click="clearUploadedChickenMedia"
          >
    ...
    <div v-if="uploadedChickenImages.length === 0" class="upload-media-notice">
      <div class="save-text">Upload an image to render chickens this session. Uploaded images are temporary and are not saved anywhere.</div>
      <button type="button" class="save-button" @click="triggerChickenMediaUpload">
    ...
    <input
      ref="chickenMediaInput"
      type="file"
      accept="image/*"
      class="save-import-input"
      multiple
      @change="handleChickenMediaUploadSelection"
    >
